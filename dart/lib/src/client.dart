...
class BucketlistValidationException implements Exception {
  final List<String> errors;

  BucketlistValidationException(this.errors);

  @override
  String toString() => 'BucketlistValidationException: ${errors.join("; ")}';
}
...
/// Bucket List API client.
///
/// For more information, visit: https://apiverve.com/marketplace/bucketlist?utm_source=dart&utm_medium=readme
///
/// Parameters:
...
class BucketlistClient {
  ...
  BucketlistClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/bucketlist',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [BucketlistValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw BucketlistValidationException(errors);
    }
  }

  /// Execute a request to the Bucket List API.
  ///
  /// Parameters are validated before sending the request.
  Future<BucketlistResponse> execute() async {
    if (apiKey.isEmpty) {
      throw BucketlistException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return BucketlistResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw BucketlistException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw BucketlistException('Resource not found');
      } else {
        throw BucketlistException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is BucketlistException) rethrow;
      throw BucketlistException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Bucket List API client.
class BucketlistException implements Exception {
  final String message;

  BucketlistException(this.message);

  @override
  String toString() => 'BucketlistException: $message';
}
